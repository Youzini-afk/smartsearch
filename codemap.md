...
- `src/smart_search/admin/routes.py`: Productized Jinja2 admin console and JSON APIs for token management, provider credential management, dedicated capability configuration (`/admin/config`), analytics dashboard, usage charts, audit, system, and task management. Config UI distinguishes effective service config from DB-backed override drafts until cloud provider injection is wired into execution. Login page supports API key and password auth (SMART_SEARCH_ADMIN_PASSWORD / _PASSWORD_HASH). HTML pages redirect to /admin/login on unauthenticated access.
