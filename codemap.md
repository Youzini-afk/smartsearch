...
- `src/smart_search/admin/routes.py`: Productized Jinja2 admin console and JSON APIs for token management, provider credential management, dedicated capability configuration (`/admin/config`), analytics dashboard, usage charts, audit, system, and task management. Config UI distinguishes effective service config from DB-backed override drafts until cloud provider injection is wired into execution; saving config synchronizes stored overrides by capability so blank fields explicitly inherit runtime config. Login page supports API key and password auth (SMART_SEARCH_ADMIN_PASSWORD / _PASSWORD_HASH). HTML pages redirect to /admin/login on unauthenticated access.
...
| `src/smart_search/admin/` | Productized Jinja2 admin WebUI/API for API keys, provider credentials, dedicated capability config, analytics, usage, audit, system, and tasks. | See `src/smart_search/codemap.md`. |
| `src/smart_search/runtime/capabilities.py` | Canonical capability metadata for admin config surfaces: effective runtime sources, stored-only override status, capability IDs, and default provenance. | See `src/smart_search/codemap.md`. |
